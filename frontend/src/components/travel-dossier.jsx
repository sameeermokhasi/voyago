...
export function TravelDossier({ plan, destination, days, budget, onBookCab }) {
  ...
  const baseWeather = cityData.weather || {
    c: 20,
    low: 10,
  ...
  const weatherVariations = [
    ...
    {
      cond: "Clear blue skies & crisp sunshine",
      high: baseWeather.c + 1,
      low: baseWeather.low,
      rain: "2%",
      icon: Sun,
    },
    {
      cond: "Mild sunny afternoon with cool evening",
      high: baseWeather.c,
      low: baseWeather.low - 1,
      rain: "5%",
      icon: Sun,
    },
    {
      cond: "Light mountain breeze & passing clouds",
      high: baseWeather.c - 1,
      low: baseWeather.low,
      rain: "8%",
      icon: CloudRain,
    ...
    {
      cond: "Crisp morning & golden sunset",
      high: baseWeather.c + 1,
      low: baseWeather.low + 1,
      rain: "2%",
      icon: Sun,
    ...
    {
      cond: "Clear starry night & mild mountain day",
      high: baseWeather.c,
      low: baseWeather.low,
      rain: "2%",
      icon: Sun,
    ...
    {
      cond: "Fresh alpine breeze & golden daylight",
      high: baseWeather.c - 1,
      low: baseWeather.low - 1,
      rain: "5%",
      icon: CloudSun,
  ...
  const dailyWeatherList = plan.realWeather
    ? plan.realWeather.map((w) => ({ ...w, icon: CloudSun }))
  ...
  // Real Stays from WebSocket
  const stays = plan.realStays || [];
  ...
  const weatherMarkdown = dailyWeatherList
    .map(
      (w) =>
        `* **Day ${w.dayNum} (${w.dateStr})**: ${w.high}°C / ${w.low}°C — ${w.cond} (Rain probability: ${w.rain})`,
    )
    .join("\n");

  const staysMarkdown = stays.length > 0
    ? `## 🏨 2. TOP 3 ${tier.key === 'luxury' ? 'LUXURY' : tier.key === 'better' ? 'COMFORT' : 'BUDGET'} STAYS IN ${destTitle.toUpperCase()}
${stays
  .map(
    (s) => `### ${s.id}. ${s.name} (${s.priceRange})
* **Type**: ${s.cuisine}
* **Location**: ${s.location} | **Status**: ${s.rating}
* **Map**: ${s.itemsWithPrice}
* *${s.quote}*`,
  )
  .join("\n")}
`
  ...
  const restaurantsMarkdown = restaurants
    .map(
...
`,
  ...
  const timelineMarkdown = dynamicTimeline
    .map(
      (t) => `### ${t.title} (${t.cost})
...
## 🍽️ 3. ${tier.heading} IN ${destTitle.toUpperCase()}
${restaurantsMarkdown}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-400 font-medium mt-0.5">
                Duration: {numDays} Days / {numDays - 1} Nights • Total Budget:{" "}
                {formatINR(totalBudget)} • {tier.label}
              </p>
            </div>
          </div>
      ...
      >
        ...
        <div className="border-b-2 border-black pb-4 flex items-center justify-between print-avoid-break">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-3 text-xs text-zinc-600 font-bold mt-1">
              ...
              <span>•</span>
              <span className="text-black bg-yellow-400 px-2 py-0.5 rounded font-black">
                {tier.label}
              </span>
            </div>
          </div>
        ...
        {stays.length > 0 && (
          <div className="space-y-4 print-avoid-break">
            <div className="flex items-center justify-between">
              <h3 className="flex items-center gap-2 font-black text-sm md:text-base uppercase tracking-wider text-black">
                <Hotel className="size-4 text-black" />
                2. TOP 3 {tier.key === 'luxury' ? 'LUXURY' : tier.key === 'better' ? 'COMFORT' : 'BUDGET'} STAYS IN {destTitle.toUpperCase()}
              </h3>
            </div>
              ...
              {stays.map((stay) => (
                ...
                >
                  <div>
                    ...
                    <h4 className="mt-2 font-black text-base text-zinc-900 leading-snug">
                      {stay.name}
                    </h4>
                    ...
                    <div className="mt-3 rounded-lg border border-zinc-200 bg-white p-2.5">
                      <a
                        href={
                          stay.itemsWithPrice.match(/\((.*?)\)/)?.[1] || "#"
                        }
                        target="_blank"
                        rel="noreferrer"
                        className="text-xs font-semibold text-blue-600 hover:underline"
                      >
                        Check Availability on Google Travel
                      </a>
                    </div>
                  </div>
        ...
        {/* 3. TIER-AWARE TOP 3 RESTAURANTS (AFFORDABLE, COMFORT, OR LUXURY) */}
        <div className="space-y-4 print-avoid-break">
          <div className="flex items-center justify-between">
            <h3 className="flex items-center gap-2 font-black text-sm md:text-base uppercase tracking-wider text-black">
              <Utensils className="size-4 text-black" />
              {stays.length > 0 ? "3" : "2"}. {tier.heading} (PLACES TO EAT IN{" "}
              {destTitle.toUpperCase()})
            </h3>
        ...
        {/* 4. CURATED LOCAL EVENTS */}
        <div className="space-y-4 print-avoid-break">
          <h3 className="flex items-center gap-2 font-black text-sm md:text-base uppercase tracking-wider text-black">
            <Calendar className="size-4 text-black" />
            {stays.length > 0 ? "4" : "3"}. CURATED LOCAL EVENTS HAPPENING IN{" "}
            {destTitle.toUpperCase()} THIS WEEK
          </h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {events.map((ev) => (
              <div
                key={ev.title}
                className="rounded-2xl border border-zinc-200 bg-zinc-50/70 p-5 shadow-sm space-y-3"
              >
                <div className="flex items-center justify-between">
        ...
        <div className="space-y-3 print-avoid-break">
          <h3 className="flex items-center gap-2 font-black text-sm md:text-base uppercase tracking-wider text-black">
            ...
            {stays.length > 0 ? "5" : "4"}. ITEMIZED FINANCIAL & TRANSIT
            BREAKDOWN ({tier.label.toUpperCase()})
          </h3>
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-zinc-200 text-zinc-700">
                ...
                <tr>
                  ...
                  <td className="p-3">
                    {tier.stayLabel} ({numDays - 1} Nights)
                  </td>
                  <td className="p-3 text-right font-black text-black">
                    {formatINR(stayCost)}
                  </td>
                ...
                <tr>
                  ...
                  <td className="p-3">
                    Curated Tier Gastronomy, Signature Tastings & Banquets
                  </td>
                  <td className="p-3 text-right font-black text-black">
                    {formatINR(foodCost)}
                  </td>
                ...
                <tr>
                  ...
                  <td className="p-3">
                    Monument passes, national park safaris, bespoke cultural
                    permits
                  </td>
                  <td className="p-3 text-right font-black text-black">
                ...
                <tr className="bg-yellow-400/20 font-black">
                  ...
                  <td className="p-3 text-black">
                    Comfortably within your {formatINR(totalBudget)} allocated
                    budget
                  </td>
                  <td className="p-3 text-right text-base text-black font-black">
                    {formatINR(estimatedSpend)}
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        {/* 6. STRUCTURED TIMELINE & CAB ITINERARY */}
        <div className="space-y-4 print-avoid-break">
          <h3 className="flex items-center gap-2 font-black text-sm md:text-base uppercase tracking-wider text-black">
            <Clock className="size-4 text-black" />
            {stays.length > 0 ? "6" : "5"}. STRUCTURED {numDays}-DAY REGIONAL
            TIMELINE & ITINERARY
          </h3>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between border-t border-zinc-200 pt-6 text-[11px] font-semibold text-zinc-400 print-avoid-break">
          <span>
            Official Indian Travel Dossier • Generated by Voyago VOYO Concierge
          </span>
