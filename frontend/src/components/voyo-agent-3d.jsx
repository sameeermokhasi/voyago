...
export function VoyoAgent3D({ state = 'idle' }) {
  const root = useRef(null)
  const torso = useRef(null)
  const head = useRef(null)
  const rArm = useRef(null)
  const rElbow = useRef(null)
  ...
  useFrame((s, delta) => {
    ...
    if (antenna.current) {
      const mat = antenna.current.material
